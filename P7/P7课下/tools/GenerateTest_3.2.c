...
int bufferType[30]; char printBuffer[30][105]; int rightAddr=0,wrongAddr=0;
int fprtot=1,tagtot,buffertot,lanum,laper,trynum,afterHandle,nownum,uset,stack[10];
...
struct mips_hardWare
{
	...
	int c0[4],grf[32],dm[4096];
	int tmpc0[4],tmpgrf[32],tmpdm[4096];
	//-------------------------
	inline void update()
	{
		hi=tmphi,lo=tmplo,pc=tmppc;
		for (int i=0;i<4;i++) c0[i]=tmpc0[i];
	...
	inline void back()
	{
		...
		for (int i=0;i<4;i++) tmpc0[i]=c0[i];
	...
	{
		hi=lo=0; pc=12288;
		c0[0]=c0[1]=c0[2]=0; c0[3]=19260817;
...
struct mips_rTypeSet//id=0
{
	...
	{
		if (a[0]==0 && addOverFlow(hardWare.tmpgrf[a[2]],hardWare.tmpgrf[a[3]],0)) 
			{hardWare.tmppc+=4; afterHandle=1; return true; /*return false;*/}
		else if (a[0]==10 && addOverFlow(hardWare.tmpgrf[a[2]],-hardWare.tmpgrf[a[3]],1)) 
			{hardWare.tmppc+=4; afterHandle=1; return true; /*return false;*/}
...
struct mips_i1TypeSet//id=1
{
	...
	inline bool trydo(int a[])
	{
		...
		if (a[0]==0 && addOverFlow(hardWare.tmpgrf[a[2]],a[3],2))
			{hardWare.tmppc+=4; afterHandle=1; return true; /*return false;*/}
		doit(a[0],a[1],a[2],a[3]),hardWare.tmppc+=4; return true;
	}
	//-------------------------
...
struct mips_lTypeSet//id=3
{
	...
	inline bool trydo(int a[])
	{
		a[3]=((rand()%40)<<2)-hardWare.tmpgrf[a[2]];
		if ((0<=(hardWare.tmpgrf[a[2]]+a[3]) && (hardWare.tmpgrf[a[2]]+a[3])<=0x2fff) &&
			...
			doit(a[0],a[1],a[2],a[3]),hardWare.tmppc+=4,rightAddr++;
		else
		{
			hardWare.tmppc+=4; wrongAddr++; a[3]=rand();
			if (DEBUGMOOD) fprintf(out,"failed %s $%d,%d($%d)\n",
			instruction[a[0]],a[1],a[3],a[2]); afterHandle=1; //return false;
		}
		return true;
	}
	//-------------------------
...
struct mips_mtTypeSet//id=5
{
	...
	inline void doit(int num,int dst,int srcA,int srcB)
	{
		...
		else if (num==6) hardWare.tmpc0[dst-12]=hardWare.tmpgrf[srcA];
		return;
	}
	//-------------------------
	inline void debug()
	...
	inline bool trydo(int a[])
	{
		...
		a[2]=rand()%30+1; a[3]=(rand()&1)+12; doit(a[0],a[3],a[1],a[2]);
		hardWare.tmppc+=4; return true;
	}
	//-------------------------
	inline int select()
	{
	...
	inline void print(int a[])
	{
		...
		else if (a[0]==4 || a[0]==5) sprintf(printBuffer[buffertot++],"%s%s$%d\n",instruction[a[0]],
		strlen(instruction[a[0]])>3?"\t":"\t\t",a[1]);
		else if (a[0]==6) sprintf(printBuffer[buffertot++],"%s%s$%d,$%d\n",instruction[a[0]],
		strlen(instruction[a[0]])>3?"\t":"\t\t",a[1],a[3]);
	}
} mtTypeSet;
...
struct mips_mfTypeSet//id=6
{
	...
	//-------------------------
	inline void doit(int num,int dst,int src)
	{
		if (num==0) hardWare.tmpgrf[dst]=hardWare.tmphi;
		else if (num==1) hardWare.tmpgrf[dst]=hardWare.tmplo;
		else if (num==2) hardWare.tmpgrf[dst]=hardWare.tmpc0[src-12];
		hardWare.tmpgrf[0]=0; return;
	}
	//-------------------------
	inline bool trydo(int a[])
	{
		hardWare.tmppc+=4; a[2]=(rand()&1)+12;
		doit(a[0],a[1],a[2]);
		return true;
	}
	//-------------------------
	...
	{
		use[a[0]]++; toRand=0; memset(vis,0,sizeof(vis));
		if (a[0]!=2) sprintf(printBuffer[buffertot++],"%s%s$%d\n",instruction[a[0]],
		strlen(instruction[a[0]])>3?"\t":"\t\t",a[1]);
		else sprintf(printBuffer[buffertot++],"%s%s$%d,$%d\n",instruction[a[0]],
		strlen(instruction[a[0]])>3?"\t":"\t\t",a[1],a[2]);
	}
} mfTypeSet;
...
inline void setRegister()
{
	...
	for (int i=0;i<blockNum;i+=4)
	{
		bool jal=hasjal(i);
		setSingleReg(i,jal);
		setDoubleReg(i+1,jal,0);
		setDoubleReg(i+2,jal,0);
		setDoubleReg(i+3,jal,1);
	}
	if (DEBUGMOOD)
	{
		fputc('\n',out);
...
{
	fprintf(fpr,".ktext 0x4180\n\n%s#end",handlerBuffer[rand()%10]);
...
{
	if (num==0) strcpy(printBuffer[buffertot++],"lw\t\t$31,-12296($31)"),nownum++;
...
inline void fixHandle()
{
	hardWare.tmpgrf[1]=(rand()&63)<<2;
	fprintf(fpr,"addi\t$1,$0,%d\n",hardWare.tmpgrf[1]);
	hardWare.tmpc0[0]=hardWare.tmpgrf[1];
	fprintf(fpr,"mfc0\t$1,$12\n");
	hardWare.tmpgrf[1]=(rand()&63)<<2;
	fprintf(fpr,"addi\t$1,$0,%d\n",hardWare.tmpgrf[1]);
	hardWare.tmpc0[1]=hardWare.tmpgrf[1];
	fprintf(fpr,"mfc0\t$1,$13\n");
	hardWare.tmpgrf[1]=(rand()&63)<<2;
	fprintf(fpr,"addi\t$1,$0,%d\n",hardWare.tmpgrf[1]);
	hardWare.tmpc0[2]=hardWare.tmpgrf[1];
	fprintf(fpr,"mfc0\t$1,$14\n");
	hardWare.tmppc+=24; return;
}

inline void fixzero()
{
	if (afterHandle) fixHandle(),afterHandle=0;
	for (int i=1;i<32;i++)
	{
...
inline void analyse()
{
	fprintf(fpr,".ktext 0x4180\n\n%s#end",handlerBuffer[rand()%10]),fclose(fpr);
	printf("all %d testpoint generated!",fprtot-1);
	puts("\nwish you ak them all!");
	if (!DEBUGMOOD) return;
	fprintf(out,"%d wrong addr ans %d right addr\n",wrongAddr,rightAddr);
	rTypeSet.printuse();
	i1TypeSet.printuse();
	i2TypeSet.printuse();
	lTypeSet.printuse();
