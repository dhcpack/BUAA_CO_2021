...
char buffer[10005];
int tot,cnt,now=0x3000,fuckAddr[64];
...
int main()
{
	...
	while (gets(buffer))
	{
		...
		if (strstr(buffer,"fuck")) fuckAddr[cnt++]=now-4+(tot<<2);
	}
	...
	printf("\t\telse if (addr==interruptAddress[%d]) begin interruptAddress[%d]<=32'h7f7f7f7f; interruptCounter<=5; interrupt<=1; end\n",i,i);
	printf("%s",endBuffer); return 0;
}