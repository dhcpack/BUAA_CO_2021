...
int fprtot=1,tagtot,buffertot,lanum,laper,trynum,afterHandle,nownum,uset,seed=23333,stack[10];
...
{
	int tot=0;
	for (int i=0;i<FILETOTNUM;i++) nameChange[i]=(i==223 || i==298)?2147483647-i:++tot; if (DEBUGMOOD) return;
	for (int i=0;i<FILETOTNUM;i++) swap(&nameChange[i],&nameChange[ran()%FILETOTNUM]);
	for (int i=0;i<FILETOTNUM;i++) swap(&nameChange[i],&nameChange[ran()%FILETOTNUM]);
	return;
}
...
struct mips_mtTypeSet//id=5
{
	...
	inline void print(int a[])
	{
		...
		else if (a[0]==5) sprintf(printBuffer[buffertot++],"%s%s$%d\n",instruction[a[0]],
...
inline void analyse()
{
	fprintf(fpr,"\n%s#end",handlerBuffer),fclose(fpr);
