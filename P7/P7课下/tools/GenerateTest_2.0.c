...
FILE *out,*fpr; int fprtot=1,tagtot,buffertot,lanum,laper,hasError;
...
struct mips_hardWare
{
	...
	inline void back()
	{
		...
		for (int i=0;i<768;i++) tmpdm[i]=dm[i];
		return;
	}
	//-------------------------
...
struct mips_rTypeSet//id=0
{
	...
	inline void doit(int num,int dst,int srcA,int srcB)
	{
		...
		else if (num==6) hardWare.tmpgrf[dst]=hardWare.tmpgrf[srcA]>>(hardWare.tmpgrf[srcB]&31);
		else if (num==7) hardWare.tmpgrf[dst]=(unsigned int)hardWare.tmpgrf[srcA]>>((unsigned int)(hardWare.tmpgrf[srcB]&31));
...
inline void fixallthree()
{
	hardWare.tmpgrf[i]=(rand()&255)+1;
	fprintf(fpr,"addi\t$%d,$0,%d\n",i,hardWare.tmpgrf[i]);
	hardWare.tmpgrf[i]
	fprintf(fpr,"addi\t$%d,$0,%d\n",i,hardWare.tmpgrf[i]);
	hardWare.tmppc+=8; return;
}

inline void fixzero()
{
	if (hasError) fixallthree(); hasError=0;
	for (int i=1;i<32;i++)
	{
...
inline void selectInstr(int index)
{
	for (int num=0;num<4;num++)
	{
		if (block[index].instrType[num]==0)
		{
			...
			if (~block[index].regUse[1]) block[index].instr[num][2]=block[index].regUse[rand()&1];
			else block[index].instr[num][2]=block[index].regUse[0];
			if (~block[index].regUse[1]) block[index].instr[num][3]=block[index].regUse[rand()&1];
			else block[index].instr[num][3]=block[index].regUse[0];
		}
