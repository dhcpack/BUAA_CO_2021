...
const int jumptonew=880,totjump=6;
const int totdb=5,totuse=11,totnorm=6;
...
inline void printjump(int num)
{
	...
	{
		sprintf(buffer[buffertop++],"jal\t\tTAG_%d",rand()%150),nownum+=3,sprintf(buffer[buffertop++],"TAG_%d:",tagptr);
		sprintf(buffer[buffertop++],"la\t\t$t9,TAG_%d",tagptr++),sprintf(buffer[buffertop++],"addi\t$31,$31,-%d",(nownum-12)<<2);
	}
	else if (num==5)
	{
		sprintf(buffer[buffertop],"la\t\t$t%d,TAG_%d",uset,rand()%150),forwardtag[buffertop++]=1;
		sprintf(buffer[buffertop++],"jalr\t$31,$t%d",uset++),nownum+=4,sprintf(buffer[buffertop++],"TAG_%d:",tagptr);
		sprintf(buffer[buffertop++],"la\t\t$t9,TAG_%d",tagptr++),sprintf(buffer[buffertop++],"addi\t$31,$31,-%d",(nownum-12)<<2);
	}
	return;
}
