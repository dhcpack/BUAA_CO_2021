...
#include <string.h>
#define SPLITTEST 0
#define PROJECTNUMBER 6

bool isAK=1; const int time=20000,pointnum=8;
...
inline void solve(const char* s)
{
	...
	if (PROJECTNUMBER==4)
		...
		sprintf(buffer1,"java -jar mars.jar a nc mc CompactDataAtZero dump 0x00004180-0x00004400 HexText code_handler.txt %s",s),system(buffer1);
		sprintf(buffer1,"java -jar mars.jar db nc mc CompactDataAtZero dump .text HexText code.txt >ans.txt %s",s),system(buffer1);
