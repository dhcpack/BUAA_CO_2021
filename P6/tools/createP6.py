# 测试样例生成程序，包含P6要求的除add,addi,sub(因为可能产生溢出，Mars报错)外的所有指令

import random

data1 = []  # 记录顺序执行的代码
data2 = []  # 记录跳转代码  # beq bne blez bgtz bltz bgez j    jal jr jalr
interval = []  # 记录中间插入部分代码的行数

# 九个大指令类
calmudv = ['mult', 'multu', 'div', 'divu', 'madd', 'maddu', 'msub', 'msubu']
readmudv = ['mflo', 'mfhi']
setmudv = ['mtlo', 'mthi']
caldoubleregister = ['addu', 'subu', 'slt', 'sltu', 'and', 'or', 'nor', 'xor', 'sllv', 'srlv', 'srav']
shift = ['sll', 'srl', 'sra']
calregisterimm = ['addiu', 'andi', 'ori', 'xori']
saveload = ['sw', 'lw', 'sh', 'lh', 'lhu', 'sb', 'lb', 'lbu']
shifti = ['slti', 'sltiu']
lui = ['lui']
...
def print_instr(x):
    if data1[x].type == 1 or data1[x].type >= 11:
        ...
        if coin == 0:
            print("{} ${},{}($0)".format(random.choice(saveload[0:2]), data1[i].rs, random.randrange(0, 8192, 4)))
        elif coin == 1:
            print("{} ${},{}($0)".format(random.choice(saveload[2:5]), data1[i].rs, random.randrange(0, 8192, 2)))
        else:
            print("{} ${},{}($0)".format(random.choice(saveload[5:]), data1[i].rs, random.randrange(0, 8192, 1)))
    elif data1[x].type == 8:
...
if __name__ == "__main__":
    ...
    for i in range(0, total_lines):  # 随机生成顺序执行代码，保存在data1中
        data1.append(Instruction(random.randint(1, 12)))
